...
3. **Two lanes; one tool = one unit of change.** A static binary with an official image goes in
   **Lane 1** (`FROM <img>:<tag>@<digest> AS <name>` + `COPY --from=<name> …`); a tool that needs
   a runtime / package manager goes in **Lane 2** (the install block). Every bundled tool gets a
   row in [`LINTERS.md`](./LINTERS.md). See
...
- To add a tool: add it to its lane, build locally, confirm it runs on this arch, and (Lane 1)
  confirm the upstream ships `arm64`. Keep [`LINTERS.md`](./LINTERS.md) in sync.

## Code style (no separate CODESTYLE.md yet)

The surface is small (one Dockerfile, one shell script, soon some workflow YAML), so:

- **Dockerfile:** two lanes; one `FROM` + one `COPY` per Lane-1 tool; pin `tag@digest`; in Lane 2
  clean apt lists in the same layer (`rm -rf /var/lib/apt/lists/*`). The image runs as the
  non-root `lint` user and expects the repo mounted read-only at `/work`.
